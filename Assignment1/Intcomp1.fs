...
(*Note to TA's -> Several functions are not compiling due to the changes to the Let case of the expr type
since these functions weren't a part of the assignment, we haven't changed them, 
but we were unsure of whether or not to comment them out.*)
...
//Changed expressions to compile with changed definition of Let
...
let rec eval e (env : (string * int) list) : int =
    ...
    | Var x             -> lookup env x 
    | Let(x, ebody) -> 
      let env2 = List.fold(fun acc (xstring,xval )  -> (xstring, eval xval acc)::acc) env x
      eval ebody env2
...
//Changed expressions to compile with changed Let
...
//Changed Let match case
let rec freevars e : string list =
    match e with
    | CstI i -> []
    | Var x  -> [x]
    | Let(x, ebody) -> 
          List.fold(fun acc (xstring,xval )  -> union (freevars ebody, minus (freevars xval, [xstring]))) [] x
...
//Changed let match cases
let rec tcomp (e : expr) (cenv : string list) : texpr =
    match e with
    | CstI i -> TCstI i
    | Var x  -> TVar (getindex cenv x)
    | Let([], ebody) -> tcomp ebody cenv
    | Let((x, ebody) :: xs,ebody2) -> 
      let cenv1 = x :: cenv 
      TLet(tcomp (Let(xs, ebody)) cenv, tcomp ebody cenv1)
